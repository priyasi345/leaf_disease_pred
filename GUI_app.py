...
windo = Tk()
windo.configure(background='pink')
windo.title("Crop Disease Prediction App")
# width  = windo.winfo_screenwidth()
# height = windo.winfo_screenheight()
# windo.geometry(f'{width}x{height}')
windo.geometry('1120x820')
windo.iconbitmap('C:\\Users\\PREETI\\Desktop\\rice.ico')
windo.resizable(0,0)
...
def prediction():
    ...
    img = load_image(path)
    try:
        with open('C:\\Users\\PREETI\\Desktop\\model\\crop_disease_pred.pkl', 'rb') as f:
            clf = pickle.load(f)
...
ri = PIL.Image.open('C:\\Users\\PREETI\\Desktop\\rice.png')
...
pred = tk.Label(windo, text="Crop Disease Prediction", width=30, height=2, fg="white",bg="dark green",
                font=('times', 25, ' bold '))
